...
//al a[5], por en, el valor hexadecimal que posee a[5] esta encima del de a[4] y justamente es por la cantidad de bytes que se le 
//designan a cada entero, por ello es que va de 4 en 4 bytes. Esto se comprueba con correr el programa 1, mostrando la diferencia de 
//que al elemento a[5] esta 4 bytes encima del a[4], pero es necesario recalcar que no necesariamente es siempre asi, es asi debido 
//a que el código tiene una estructura que hace que asi sean las cosas siempre, pero se puede modificar para que el llenado sea diferente 
//y por consiguiente la dirección del elemento a[5] puede ser menor que el del elemento a[4], pero en este caso no aplica.
//****************************NOTA*******************************
